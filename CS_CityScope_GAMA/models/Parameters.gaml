...
global {
	...
	float clusterDistance <- 100#m; //Radius in which we look for bikes to cluster with
	float clusterThreshold <- 10000.0; //(see bike.clusterCost) the charge a foller must be able to give the leader in order to cluster
    ...
    float peopleSpeed <- 1.0 #km/#h	min: 0.5#km/#h max: 10#km/#h parameter: "People Speed (m/s):" category: "People";
